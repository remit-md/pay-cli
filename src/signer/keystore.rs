//! Encrypted key storage with scrypt KDF.
//!
//! Security invariants:
...
//!   decryption for address display)
...
impl Keystore {
    /// Open the keystore at the default location (`~/.pay/keys/`).
    ///
    /// Uses `PAY_KEYS_DIR` env override if set (for testing).
    pub fn open() -> Result<Self> {
        let dir = if let Ok(d) = std::env::var("PAY_KEYS_DIR") {
            PathBuf::from(d)
        } else {
            dirs::home_dir()
    ...
    pub fn generate(&self, name: &str, passphrase: &str) -> Result<String> {
        ...
        getrandom::fill(&mut *raw).map_err(|e| anyhow!("rng failed: {e}"))?;
        let key = SigningKey::from_slice(&*raw)
            .map_err(|_| anyhow!("failed to create signing key from random bytes"))?;
        let address = auth::derive_address(&key);

        // Encrypt and write
        let encryption = encrypt_key(&*raw, passphrase)?;
    ...
    /// Load an encrypted key file from disk (does not decrypt).
...
/// Encrypt a private key (must be 32 bytes) with AES-256-GCM.
///
...
pub fn encrypt_key(private_key: &[u8], passphrase: &str) -> Result<EncryptionParams> {
    ...
    getrandom::fill(&mut salt).map_err(|e| anyhow!("rng failed: {e}"))?;
    getrandom::fill(&mut nonce_bytes).map_err(|e| anyhow!("rng failed: {e}"))?;
...
/// Decrypt a key file and return a `SigningKey`.
///
/// All intermediate decrypted bytes are `Zeroizing<>` -- zeroed on drop.
pub fn decrypt(key_file: &EncryptedKeyFile, passphrase: &str) -> Result<SigningKey> {
    let salt =
...
mod tests {
    ...
    fn generate_load_decrypt_roundtrip() {
        ...
        let key = decrypt(&loaded, passphrase).unwrap();
        assert_eq!(auth::derive_address(&key), address);
    }
    ...
    fn import_load_decrypt_roundtrip() {
        ...
        let key = decrypt(&loaded, passphrase).unwrap();
        assert_eq!(auth::derive_address(&key), expected_addr);
    }
    ...
    // -- Corrupted ciphertext detected (AES-GCM auth tag) --------------------
    ...
    // -- Deterministic signing from decrypted key -----------------------------

    #[test]
    fn decrypted_key_produces_valid_signatures() {
        let (ks, _dir) = test_keystore();
        let key_hex = "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
        ...
        let (sig2, recid2) = key.sign_prehash_recoverable(&hash).unwrap();
        let mut sig_bytes2 = Vec::with_capacity(65);
        sig_bytes2.extend_from_slice(&sig2.r().to_bytes());
        sig_bytes2.extend_from_slice(&sig2.s().to_bytes());
        sig_bytes2.push(recid2.to_byte() + 27);
        assert_eq!(
            sig_bytes, sig_bytes2,
