...
///
/// Default: Pay's own signer (encrypted keystore).
/// Use --ows for OWS (Open Wallet Standard) vault.
#[derive(Args)]
pub struct InitArgs {
    ...
    /// Use OWS (Open Wallet Standard) instead of Pay signer
    #[arg(long)]
    pub ows: bool,
}
...
pub async fn run(args: InitArgs, ctx: super::Context) -> Result<()> {
    ...
    // Default: Pay signer
    run_pay_signer(args, ctx).await
}

/// Default init: Pay's own encrypted signer.
async fn run_pay_signer(_args: InitArgs, _ctx: super::Context) -> Result<()> {
...
/// OWS init: create wallet in OWS vault with policy and API key.
async fn run_ows(args: InitArgs, ctx: super::Context) -> Result<()> {
    ...
    if !ows::is_ows_available() {
        error::success("OWS not detected. Installing via npm...");
        ...
        if !ows::is_ows_available() {
            return Err(anyhow::anyhow!(
                "OWS installation failed. Install manually: npm install -g @open-wallet-standard/core\n\
                 Or use `pay init` (without --ows) for the Pay signer."
            ));
        }
    ...
    error::success(&format!("Creating wallet '{wallet_name}'..."));
