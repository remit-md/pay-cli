...
async fn run_list(ctx: Context) -> Result<()> {
    ...
    if wallets.is_empty() {
        if ctx.json {
            ...
            error::success("No OWS wallets found. Run `pay init --ows` to create one.");
        }
        return Ok(());
    }
    ...
    if ctx.json {
        ...
        for w in &wallets {
            ...
            table.add_row(vec![&w.name, &address, &short_id, &w.created_at]);
        }
        println!("{table}");
    }
...
async fn run_fund(args: WalletFundArgs, ctx: Context) -> Result<()> {
    ...
    if ctx.json {
        ...
        println!();
        error::success("Open this link to add USDC to your wallet.");

        // Try to open in browser
        if open_url(&url).is_err() {
            error::success("(Could not open browser automatically)");
        }
    }
...
async fn run_set_policy(args: SetPolicyArgs, ctx: Context) -> Result<()> {
    ...
    if ctx.json {
        ...
        if has_limits {
            println!();
            error::success("Spending limits use the @pay-skill/ows-policy executable.");
            error::success("Install it: npm install -g @pay-skill/ows-policy");
        }
    }
...
async fn run_settings(args: WalletSettingsArgs, ctx: Context) -> Result<()> {
    // For now, display the requested settings change
    if ctx.json {
        error::print_json(&serde_json::json!({
